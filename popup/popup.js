 ...
 * - 8 new header toggle handling
 * - Stats refresh on open
 */
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (s.panelOpen) {
        ...
        document.getElementById('masterSwitchRow').style.display = 'flex';
    ...
    function refreshStats() {
        chrome.runtime.sendMessage({ action: 'getStats' }, (r) => {
            if (!r) return;
            document.getElementById('statHeaders').textContent = r.activeHeaders || 0;
            document.getElementById('statPayloads').textContent = r.totalPayloads || 0;
            document.getElementById('statSites').textContent = r.sitesInjected || 0;
            document.getElementById('statFills').textContent = r.sessionFills || 0;

            // Highlight active headers count
            const headerEl = document.getElementById('statHeaders');
            if (r.activeHeaders > 0) {
                headerEl.style.color = '#00ff41';
                headerEl.style.textShadow = '0 0 6px rgba(0,255,65,0.4)';
            } else {
    ...
    const masterRow = document.getElementById('masterSwitchRow');

    document.getElementById('panelToggle').addEventListener('click', () => {
        panelOpen = !panelOpen;
        togglesEl.style.display = panelOpen ? 'flex' : 'none';
        masterRow.style.display = panelOpen ? 'flex' : 'none';
    ...
    // v4.0: Master Switch handlers
    document.getElementById('masterOnBtn').addEventListener('click', () => {
        chrome.runtime.sendMessage({ action: 'masterSwitch', enabled: true }, (r) => {
            if (r?.toggles) updateHeaderUI(r.toggles);
            refreshStats();
        });
    });

    document.getElementById('masterOffBtn').addEventListener('click', () => {
        chrome.runtime.sendMessage({ action: 'masterSwitch', enabled: false }, (r) => {
            if (r?.toggles) updateHeaderUI(r.toggles);
            refreshStats();
        });
    });
    ...
    function updateHeaderUI(toggles) {
        ...
        // Update panel indicator dot
        const indicator = document.querySelector('.panel-indicator');
        if (indicator) {
            indicator.style.background = n > 0 ? '#00ff41' : '';
            indicator.style.boxShadow = n > 0 ? '0 0 6px rgba(0,255,65,0.4)' : '';
        }
    }
    ...
    document.getElementById('scanBtn').addEventListener('click', () => {
        ...
        chrome.runtime.sendMessage({ action: 'injectAndCount' }, (r) => {
            btn.disabled = false;
            btn.querySelector('.btn-text').textContent = 'SCAN FIELDS';
            if (r && !r.error) {
                lastScanResult = r;
    ...
    function updateFieldSummary(d) {
        ...
        const icons = { name: '👤', email: '📧', phone: '📞', url: '🔗', subject: '📝', message: '💬', address: '📍', company: '🏢', search: '🔍', generic: '📋', hidden: '👻' };
        let h = `<div class="field-count-badge">${d.total} fields`;
        if (d.byType?.hidden) h += ` <span class="hidden-badge">(${d.byType.hidden} hidden)</span>`;
        h += `</div><div class="field-type-grid">`;
        for (const [t, c] of Object.entries(d.byType || {}))
            h += `<div class="field-type-item ${t === 'hidden' ? 'hidden-type' : ''}"><span>${icons[t] || '📋'} ${t}</span><span class="ftc">${c}</span></div>`;
        el.innerHTML = h + '</div>';
    }
    ...
    document.getElementById('fillAllBtn').addEventListener('click', () => {
        ...
        const svgBolt = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>';
        btn.disabled = true;
        ...
        chrome.runtime.sendMessage({ action: 'injectAndFillAll' }, (r) => {
            bar.style.width = '100%';
            if (r?.filled > 0) {
                const hiddenMsg = r.hidden > 0 ? ` (${r.hidden} hidden)` : '';
                txt.textContent = `✅ ${r.filled} fields injected!${hiddenMsg}`;
                btn.querySelector('.btn-text').textContent = `${r.filled} INJECTED ✅`;
                btnIcon.innerHTML = '✅';
            } else if (r?.total === 0) {
            ...
            setTimeout(() => {
                btn.disabled = false;
                btn.querySelector('.btn-text').textContent = 'INJECT ALL';
                btnIcon.innerHTML = svgBolt;
            }, 2500);
        });
    });
    ...
    function renderExcludes(domains) {
        ...
        el.innerHTML = domains.map(d =>
            `<div class="exclude-item ${d.startsWith('*.') ? 'wildcard' : ''}">
        <span class="exclude-domain">${d.startsWith('*.') ? '🌐 ' : ''}${esc(d)}</span>
